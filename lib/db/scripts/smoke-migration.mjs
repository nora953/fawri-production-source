...
async function assertCriticalForeignKeys(pool) {
  ...
  const result = await pool.query(`
    ...
    INNER JOIN pg_class AS source_table ON source_table.oid = constraint.conrelid
    INNER JOIN pg_class AS target_table ON target_table.oid = constraint.confrelid
    INNER JOIN LATERAL unnest(constraint.conkey) WITH ORDINALITY AS source_key(attnum, position) ON true
    INNER JOIN LATERAL unnest(constraint.confkey) WITH ORDINALITY AS target_key(attnum, position)
