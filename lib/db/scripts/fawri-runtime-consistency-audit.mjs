...
try {
  ...
  if (
    ...
    const orphanedOpenShifts = (
      await client.query(`
        SELECT sh.id, sh.merchant_id, sh.station_id, sh.staff_id, sh.started_at
        FROM cashier_shifts sh
        LEFT JOIN cashier_operator_sessions s
          ON s.shift_id = sh.id
         AND s.merchant_id = sh.merchant_id
         AND s.status = 'active'
         AND s.expires_at > NOW()
        WHERE sh.status = 'open' AND s.id IS NULL
        ORDER BY sh.started_at ASC, sh.id ASC
      `)
    ).rows;
    if (orphanedOpenShifts.length) {
      add(
        'review',
        'CASHIER_OPEN_SHIFT_WITHOUT_LIVE_OPERATOR_SESSION',
        'Open cashier shifts exist without an unexpired active operator session; verify whether reconciliation should close them automatically.',
        orphanedOpenShifts,
      );
    }
  }
