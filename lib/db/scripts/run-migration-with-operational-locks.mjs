...
function countStoredOrderOperations(dataDirectory) {
  ...
  const orders =
    source?.orders && typeof source.orders === "object" && !Array.isArray(source.orders)
      ? source.orders
      : {};
  return Object.values(orders).reduce((total, operations) => {
    if (!operations || typeof operations !== "object" || Array.isArray(operations)) {
      return total;
    }
    return total + Object.keys(operations).length;
  }, 0);
}

function assertStoredOrderOperationsWritable(dataDirectory) {
  const operations = countStoredOrderOperations(dataDirectory);
  if (operations === 0) return;
  assertOperationalOverlaysWritable({
    order_operations_migration: {
      summary: { operations },
    },
  });
}
...
export function runLockedMigration({
  ...
  try {
    ...
    assertStoredOrderOperationsWritable(resolvedDataDirectory);
