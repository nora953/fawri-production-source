  ...
  try {
    ...
    if (history.length >= targetIndex + 1) {
      ...
      process.stdout.write(`${JSON.stringify({
        ...readiness,
        mode: checkOnly ? 'cashier_discount_override_preflight' : 'cashier_discount_override_apply',
        database,
        target: targetEntry,
      ...
      if (checkOnly) {
        ...
        process.stdout.write(`${JSON.stringify({
          ok: true,
          mode: 'cashier_discount_override_preflight',
          database,
          target: targetEntry,
          target_sql_sha256: targetSqlSha256,
          ...
          out_of_band_schema_present: false,
          database_writes_performed: false,
        }, null, 2)}\n`);
      }
    }
