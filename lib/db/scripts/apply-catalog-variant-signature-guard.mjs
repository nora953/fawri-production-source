...
assert.equal(entries.length, targetIndex + 1, `${TARGET_TAG} must be the final committed migration for this guarded apply`);
...
try {
  ...
  try {
    ...
    if (history.length === entries.length) {
      assert.equal(history.at(-1)?.created_at, targetEntry.when, "target migration is not the latest applied migration");
  ...
  if (!alreadyApplied) {
    migrationsFolder = createStabilizedMigrationFolder(migrationsRoot);
    ...
    try {
      ...
      assert.equal(historyAfter.length, entries.length, "migration history count did not advance to 13");
      assert.equal(historyAfter.at(-1)?.created_at, targetEntry.when, "0012 was not recorded as the latest migration");
