...
function run(command, args, options = {}) {
  const result = spawnSync(command, args, { cwd: dbRoot, stdio: "inherit", env: process.env, ...options });
...
function bootstrapStageOneIfNeeded() {
  ...
  if (!process.env.GITHUB_ACTIONS || !process.env.GITHUB_TOKEN) {
    throw new Error("Generated 0002 bootstrap is required; run in GitHub Actions with read-only GITHUB_TOKEN");
  }
  ...
  run("curl", ["--fail", "--location", "--silent", "--show-error", "-H", `Authorization: Bearer ${process.env.GITHUB_TOKEN}`, "-H", "X-GitHub-Api-Version: 2022-11-28", `https://api.github.com/repos/nora953/fawri-production-source/actions/artifacts/${bootstrap.id}/zip`, "-o", zipPath]);
