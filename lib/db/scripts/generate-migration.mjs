...
function loadAppendStages() {
  ...
  const stages = fs
    ...
    .map((index) => {
      ...
      const mode = String(metadata.mode || "drizzle");
      if (!["drizzle", "manual_sql", "reviewed_sql"].includes(mode)) {
        throw new Error(`Append migration stage ${index} has unsupported mode ${mode}`);
      }
      ...
      if (new Set(preimageFiles).size !== preimageFiles.length) {
        throw new Error(
          `Append migration stage ${index} must declare unique preimage_files`,
        );
      }
      if (mode !== "manual_sql" && preimageFiles.length === 0) {
        throw new Error(
          `${mode} migration stage ${index} must declare preimage_files`,
        );
      }
      ...
      const manualSqlPath = path.join(directory, "manual.sql");
      if (mode === "manual_sql" && !fs.existsSync(manualSqlPath)) {
        throw new Error(
          `Manual migration stage ${index} is missing manual.sql`,
        );
      }
      const reviewedSqlSha256 =
        mode === "reviewed_sql"
          ? String(metadata.sql_sha256 || "").trim().toLowerCase()
          : null;
      const reviewedSqlPath =
        mode === "reviewed_sql"
          ? path.join(
              committedDrizzleDir,
              migrationFileName({ index, name: String(metadata.name || "") }),
            )
          : null;
      if (mode === "reviewed_sql") {
        if (!/^[a-f0-9]{64}$/.test(reviewedSqlSha256 || "")) {
          throw new Error(
            `Reviewed SQL migration stage ${index} must declare a valid sql_sha256`,
          );
        }
        if (!reviewedSqlPath || !fs.existsSync(reviewedSqlPath)) {
          throw new Error(
            `Reviewed SQL migration stage ${index} is missing committed SQL`,
          );
        }
        const actualHash = sha256(reviewedSqlPath);
        if (actualHash !== reviewedSqlSha256) {
          throw new Error(
            `Reviewed SQL migration stage ${index} hash mismatch; expected=${reviewedSqlSha256}, actual=${actualHash}`,
          );
        }
      }
      return {
        ...
        preimageFiles,
        preimageDirectory,
        manualSqlPath: mode === "manual_sql" ? manualSqlPath : null,
        reviewedSqlPath,
        reviewedSqlSha256,
      };
    });
...
function appendPreimageHashes(appendStages) {
  return Object.fromEntries(
    appendStages.map((stage) => {
      ...
      if (stage.mode === "manual_sql" && stage.manualSqlPath) {
        hashes["manual.sql"] = sha256(stage.manualSqlPath);
      }
      if (stage.mode === "reviewed_sql" && stage.reviewedSqlPath) {
        hashes["reviewed.sql"] = sha256(stage.reviewedSqlPath);
      }
      return [stage.index, hashes];
    }),
  );
}
...
function inspectCommittedState(expected) {
  ...
  let missingSnapshotStartIndex = null;
  entries.forEach((_, index) => {
    ...
    if (fs.existsSync(snapshotPath)) {
      if (missingSnapshotStartIndex !== null) {
        throw new Error(
          `Committed migration snapshots contain a gap before ${snapshotFileName(index)}`,
        );
      }
      return;
    }
    ...
    if (missingSnapshotStartIndex === null) {
      missingSnapshotStartIndex = index;
    }
  });
  ...
  return {
    entries,
    sql,
    missingSnapshotStartIndex,
    complete:
      entries.length === expected.length &&
      missingSnapshotStartIndex === null,
  };
}
...
function assertCommittedReproducible(
  ...
  return {
    missing_snapshot_start_index: committedState.missingSnapshotStartIndex,
  };
}
...
function preserveReviewedSql({ stage, tempOutput }) {
  if (!stage.reviewedSqlPath || !stage.reviewedSqlSha256) {
    throw new Error(`Reviewed SQL migration stage ${stage.index} has no pinned SQL source`);
  }
  const reviewedHash = sha256(stage.reviewedSqlPath);
  if (reviewedHash !== stage.reviewedSqlSha256) {
    throw new Error(
      `Reviewed SQL migration stage ${stage.index} hash changed; expected=${stage.reviewedSqlSha256}, actual=${reviewedHash}`,
    );
  }
  const generatedPath = path.join(tempOutput, migrationFileName(stage));
  if (!fs.existsSync(generatedPath)) {
    throw new Error(
      `Reviewed SQL migration stage ${stage.index} did not generate ${migrationFileName(stage)}`,
    );
  }
  const generatedHash = sha256(generatedPath);
  fs.copyFileSync(stage.reviewedSqlPath, generatedPath);
  const preservedHash = sha256(generatedPath);
  if (preservedHash !== stage.reviewedSqlSha256) {
    throw new Error(
      `Reviewed SQL migration stage ${stage.index} could not preserve canonical SQL`,
    );
  }
  const details = {
    generated_sql_sha256: generatedHash,
    reviewed_sql_sha256: preservedHash,
  };
  emitDiagnostic("reviewed-sql-stage", {
    migration_index: stage.index,
    migration_name: stage.name,
    ...details,
  });
...
function generateManualStage({ stage, tempOutput }) {
  if (!stage.manualSqlPath) {
    throw new Error(`Manual migration stage ${stage.index} has no SQL source`);
  }
  const beforeEntries = journalEntries(tempOutput).length;
  const beforeSql = sqlFiles(tempOutput);
  const expectedSql = migrationFileName(stage);
  const previousSnapshotPath = path.join(
    tempOutput,
    "meta",
    snapshotFileName(stage.index - 1),
  );
  if (!fs.existsSync(previousSnapshotPath)) {
    throw new Error(
      `Manual migration stage ${stage.index} is missing previous snapshot`,
    );
  }

  fs.copyFileSync(stage.manualSqlPath, path.join(tempOutput, expectedSql));
  const previousSnapshot = JSON.parse(
    fs.readFileSync(previousSnapshotPath, "utf8"),
  );
  const nextSnapshot = {
    ...previousSnapshot,
    id: "<generated-id>",
  ...
  const value = journal(tempOutput);
  const entries = Array.isArray(value.entries) ? value.entries : [];
  entries.push({
    idx: stage.index,
    version: "7",
  ...
  if (
    ...
    throw new Error(
      `Manual migration ${stage.index} must create only ${expectedSql}; journal ${beforeEntries}->${afterEntries}, files=${JSON.stringify(
        createdSql,
      )}`,
    );
  }
  ...
  const snapshot = canonicalizeSnapshot(tempOutput, stage.index);
  canonicalizeJournalEntry(tempOutput, stage);
  emitDiagnostic("manual-stage", {
    migration_index: stage.index,
    migration_name: stage.name,
    sql_sha256: sha256(stage.manualSqlPath),
...
try {
  ...
  for (const stage of appendStages) {
    let reviewedSql = null;
    const snapshot =
      stage.mode === "manual_sql"
        ? generateManualStage({ stage, tempOutput })
        : generateStage({
            stage,
            tempSchema,
            tempOutput,
    ...
    if (stage.mode === "reviewed_sql") {
      reviewedSql = preserveReviewedSql({ stage, tempOutput });
    }
    appendSnapshots.push({
      index: stage.index,
      mode: stage.mode,
      ...snapshot,
      ...(reviewedSql ? { reviewed_sql: reviewedSql } : {}),
    });
  }
    ...
    `${JSON.stringify(
      {
        ...
        mode: externalOutput
          ...
          : committedState.complete
            ? "verify_committed"
            : "complete_committed_metadata",
        committed_reproducible: true,
        committed_metadata_complete: committedState.complete,
        historical_reproducible: true,
        missing_snapshot_start_index:
          reproducibility.missing_snapshot_start_index,
        ...
        append_stages: appendStages.map((stage) => ({
          ...
          ...(stage.reviewedSqlSha256
            ? { sql_sha256: stage.reviewedSqlSha256 }
            : {}),
        })),
