...
function comparableEntries(table, expectedRow) {
  return Object.entries(expectedRow).filter(([columnName, value]) => {
    const column = table.columns?.[columnName];
    assert.ok(column, `unknown target column ${table.name}.${columnName}`);
...
function actualRowMatches(table, expectedRow, actualRow) {
  return comparableEntries(table, expectedRow).every(([columnName, expected]) => {
    const column = table.columns[columnName];
  ...
  for (const tableName of plannedTables) {
    ...
    for (const expectedRow of rows[tableName]) {
      ...
      for (let index = 0; index < unmatched.length; index += 1) {
        try {
          if (actualRowMatches(table, expectedRow, unmatched[index])) {
            matchIndex = index;
            break;
          }
        } catch {
          // Keep searching for the matching row.
        }
      }
