...
export async function applyPaymentDecision(client, input) {
  ...
  await client.query(
    `INSERT INTO order_payment_decisions(
       ...
       $1,$2,$3,$4::payment_decision_operation,$5::payment_decision_channel,$6::payment_decision_outcome,
       $7::order_status,$8::order_status,$9::payment_status,$10::payment_status,
       $11::payment_decision_actor,$12,$13,$14,$15,$16,$17,$18,$19,$20,clock_timestamp()
     )`,
    [
      input.decisionId,
      input.merchantId,
  ...
  await client.query(
    `UPDATE orders
     SET status=$3::order_status,
         ...
         version=$5,
         payment_verified_at=CASE WHEN $4::payment_status='paid'::payment_status THEN clock_timestamp() ELSE NULL END,
         payment_verified_by_account_id=CASE WHEN $4::payment_status='paid'::payment_status THEN $6 ELSE NULL END,
         payment_rejection_reason=CASE WHEN $4::payment_status='failed'::payment_status THEN COALESCE($7,'rejected') ELSE NULL END,
