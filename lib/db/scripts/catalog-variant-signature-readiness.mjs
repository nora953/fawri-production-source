...
try {
  ...
  process.stdout.write(`${JSON.stringify({
    ok: Number(signatureStats.invalid_signature_count) === 0,
    ...
    signature_guard_trigger_present: Boolean(guard.trigger_present),
    signature_normalizer_present: Boolean(guard.normalizer_present),
    database_writes_performed: false,
  }, null, 2)}\n`);

  if (Number(signatureStats.invalid_signature_count) !== 0) process.exitCode = 2;
} catch (error) {
  try {
    await client.query('ROLLBACK');
  } catch {}
