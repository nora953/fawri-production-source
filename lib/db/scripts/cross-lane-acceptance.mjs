...
async function testIdentityConstraints() {
  await expectReject(
    () => client.query("INSERT INTO accounts(id,kind,phone,password_hash,state,language,password_version,security_version,session_version,created_at,updated_at) VALUES('dup','merchant','+9647111111111','x','active','ar',1,1,1,clock_timestamp(),clock_timestamp())"),
...
async function main() {
  ...
  const predicateSql = fs.readFileSync(
    path.join(dbRoot, "migration-stages", "0024", "manual.sql"),
    "utf8",
  );
  const firstPolicyIndex = schemaSql.search(/^(CREATE|ALTER) POLICY /m);
  assert.ok(
    firstPolicyIndex >= 0,
    "complete schema SQL must contain tenant policies for predicate bootstrap",
  );
  const bootstrappedSchemaSql =
    schemaSql.slice(0, firstPolicyIndex) +
    predicateSql +
    "\n" +
    schemaSql.slice(firstPolicyIndex);
  admin = new Client({ connectionString: adminUrl.toString() });
  await admin.connect();
  await admin.query(`CREATE DATABASE ${databaseName}`);
  client = await createClient();
  await client.query(bootstrappedSchemaSql.replaceAll("--> statement-breakpoint",""));
