...
import { type DurableJobStatus } from "../services/durableJobQueue";
import {
  listDurableJobSummariesAuthoritative,
  requeueDeadLetterJobAuthoritative,
} from "../services/postgresDurableJobQueue";
...
export function createChannelDurableJobAdminRouter(
  ...
  router.get(
    ...
    async (req: Request, res: Response) => {
      ...
      return res.json({
        ok: true,
        jobs: (await listDurableJobSummariesAuthoritative(status)).slice(0, limit),
  ...
  router.post(
    "/admin/background-jobs/:jobId/requeue",
    authorize,
    async (req: Request, res: Response) => {
      ...
      try {
        const job = await requeueDeadLetterJobAuthoritative(jobId);
        return res.json({
          ok: true,
          job: (await listDurableJobSummariesAuthoritative()).find(
            (item) => item.id === job.id,
          ),
          payloads_included: false,
        });
