...
export class AuthSecurityStore {
  ...
  rotateSession(input: {
    ...
    return this.sessions.rotate(input);
  }
  revokeSession(token: string, reason: SessionRevocationReason) {
    return this.sessions.revoke(token, reason);
  }
  revokeSessionById(input: {
    ...
    return this.sessions.revokeById(input);
  }
  revokeSessionForAccount(input: {
    accountId: string;
    accountKind: AccountKind;
    sessionId: string;
    reason: SessionRevocationReason;
  }) {
    return this.sessions.revokeForAccount(input);
  }
  revokeAllSessions(input: {
    ...
    return this.sessions.revokeAll(input);
  }
  listActiveSessions(accountId: string, kind: AccountKind) {
    return this.sessions.list(accountId, kind);
  }
  ...
  isDeviceTrusted(accountId: string, kind: AccountKind, deviceId: string) {
    return this.devices.trusted(accountId, kind, deviceId);
  }
  setDeviceTrust(input: {
    deviceRecordId: string;
    trusted: boolean;
    actorAccountId: string;
  }) {
    return this.devices.setTrust(input);
  }
  listDevices(accountId?: string) {
    return this.devices.list(accountId);
  }
  issueOtpChallenge(input: {
    target: string;
    purpose: OtpPurpose;
    ip: string;
  }) {
    return this.otp.issue(input);
  }
  verifyOtpChallenge(input: {
    challengeId: string;
    target: string;
    purpose: OtpPurpose;
    code: string;
    ...
    return this.otp.verify(input);
  }
  revokeOtpChallenge(challengeId: string) {
    this.otp.revoke(challengeId);
  }
  checkLoginAllowed(input: {
    target: string;
    accountKind: AccountKind;
    ip: string;
  }) {
    return this.otp.checkLogin(input);
  }
  recordLoginAttempt(input: {
    ...
    this.otp.recordLogin(input);
  }
  audit(input: Omit<SecurityAuditEvent, "id" | "created_at">) {
    this.otp.audit(input);
  }
  readAuditEvents() {
    return this.otp.auditEvents();
  }
}
