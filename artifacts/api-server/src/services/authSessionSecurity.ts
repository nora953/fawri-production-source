...
export class AuthSessionSecurity {
  ...
  issue(input: {
    ...
    const data = this.db.read(), now = this.db.now();
    this.db.clean(data, now);
    const active = data.sessions.filter(
      (x) =>
        x.account_id === input.accountId &&
        x.account_kind === input.accountKind &&
        !x.revoked_at &&
        Date.parse(x.idle_expires_at) > now &&
        Date.parse(x.absolute_expires_at) > now,
    );
    const cap = input.accountKind === "admin" ? 2 : 5;
    while (active.length >= cap) {
      const oldest = active.shift()!;
      oldest.revoked_at = new Date(now).toISOString();
      oldest.revoked_reason = "manual_revocation";
    }
    const material = this.db.token(), created = new Date(now).toISOString();
    const absolute =
      input.absoluteExpiresAt && Date.parse(input.absoluteExpiresAt) > now
        ? input.absoluteExpiresAt
        : new Date(now + this.db.sessionAbsoluteTtlMs).toISOString();
    const session: AuthSessionRecord = {
      ...
      account_version: Number.isInteger(input.accountVersion)
        ? Number(input.accountVersion)
        : 0,
      ...(input.adminRole ? { admin_role: input.adminRole } : {}),
      permissions: normalizeAdminPermissions(input.permissions),
      ...(input.deviceId
        ? { device_hash: this.db.fingerprint("device", input.deviceId) }
        : {}),
      device_label: String(input.deviceLabel || "Unknown device")
        .replace(/\s+/g, " ")
        .slice(0, 120),
      created_at: created,
      last_seen_at: created,
      idle_expires_at: new Date(
        Math.min(now + this.db.sessionTtlMs, Date.parse(absolute)),
      ).toISOString(),
  ...
  validate(input: {
    ...
    if (
      !session ||
      session.revoked_at ||
      session.account_kind !== input.expectedKind ||
      !safeEqual(
        session.token_hash,
        this.db.fingerprint("session", parsed.secret),
      )
    ) {
      return null;
    }
    ...
    if (session.device_hash) {
      if (
        ...
        !safeEqual(
          session.device_hash,
          this.db.fingerprint("device", input.deviceId),
        )
      ) {
        return null;
      }
      const trusted = data.devices.some(
        (x) =>
          x.account_id === session.account_id &&
          x.account_kind === session.account_kind &&
          x.device_hash === session.device_hash &&
          x.status === "trusted",
      );
      if (session.account_kind === "admin" && !trusted) return null;
    }
    session.last_seen_at = new Date(now).toISOString();
    session.idle_expires_at = new Date(
      Math.min(now + this.db.sessionTtlMs, Date.parse(session.absolute_expires_at)),
    ).toISOString();
    this.db.write(data);
    return {
      session,
      needsRotation: Date.parse(session.rotate_after) <= now,
    };
  }
  ...
  rotate(input: {
    ...
    const issued = this.issue({
      ...
      accountKind: old.account_kind,
      tenantId: old.tenant_id,
      accountVersion: old.account_version,
      adminRole: old.admin_role,
      permissions: old.permissions,
      deviceId: input.deviceId,
      deviceLabel: old.device_label,
      absoluteExpiresAt: old.absolute_expires_at,
    });
    const data = this.db.read();
    const current = data.sessions.find((x) => x.id === old.id);
    if (!current || current.revoked_at) {
      this.revoke(issued.token, "manual_revocation");
      return null;
    }
    current.revoked_at = new Date(this.db.now()).toISOString();
  ...
  revoke(token: string, reason: SessionRevocationReason): boolean {
    ...
    session.revoked_at = new Date(this.db.now()).toISOString();
  ...
  revokeForAccount(input: {
    ...
    session.revoked_at = new Date(this.db.now()).toISOString();
    session.revoked_reason = input.reason;
    this.db.write(data);
    return true;
  }
  ...
  revokeAll(input: {
    ...
    for (const session of data.sessions) {
      if (
        session.account_id === input.accountId &&
        session.account_kind === input.accountKind &&
        !session.revoked_at &&
        (!input.deviceHash || session.device_hash === input.deviceHash)
      ) {
