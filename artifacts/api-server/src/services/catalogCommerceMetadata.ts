...
function serviceLocationChoices(
  ...
  if (value === undefined || value === null || value === "") {
    ...
    return fallbackMode === "flexible"
      ? [...ALL_SERVICE_LOCATION_CHOICES]
      : [fallbackMode];
  }
  if (!Array.isArray(value) || value.length === 0 || value.length > ALL_SERVICE_LOCATION_CHOICES.length) {
    throw new CatalogRuntimeError(
      "CATALOG_SERVICE_LOCATION_INVALID",
      "service location_modes must contain one to three valid locations",
      400,
      { field: "service_details.location_modes" },
    );
  }
  ...
  for (const rawChoice of value) {
    ...
    if (!SERVICE_LOCATION_CHOICES.has(choice)) {
      throw new CatalogRuntimeError(
        "CATALOG_SERVICE_LOCATION_INVALID",
        "service location_modes contains an invalid location",
        400,
        { location: choice },
      );
    }
  ...
  if (choices.length === 0) {
    throw new CatalogRuntimeError(
      "CATALOG_SERVICE_LOCATION_INVALID",
      "service location_modes must contain at least one location",
      400,
    );
  }
  return choices;
...
function serviceLocationModeFromChoices(
...
export function catalogCommerceFromMetadata(metadataValue: unknown): CatalogCommerceFields {
  ...
  const storedLocationModes = serviceLocationChoices(
    rawService.location_modes,
    storedLocationMode,
  );
...
export function normalizeCatalogCommerceInput(
  ...
  const locationModes = hasOwn(serviceInput, "location_modes")
    ? serviceLocationChoices(
        serviceInput.location_modes,
        requestedLocationMode,
        existingService?.location_modes,
      )
