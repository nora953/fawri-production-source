...
export type EmergencySnapshotContext = {
  request_id: string;
...
export class EmergencySnapshotPostgresError extends Error {
  readonly code: string;
  readonly statusCode: number;

  constructor(code: string, message: string, statusCode = 400) {
    super(message);
    this.name = "EmergencySnapshotPostgresError";
    this.code = code;
    this.statusCode = statusCode;
  }
}

export async function buildEmergencyReadOnlySnapshotPostgres(input: {
  merchantId: string;
  emergencyAccess: EmergencySnapshotContext;
}) {
  if (!operationalPostgresAuthorityRequired()) {
    throw new EmergencySnapshotPostgresError(
  ...
  return withMerchantOperationalTransaction(merchantId, async (client) => {
    ...
    return {
      emergency_access: input.emergencyAccess,
      merchant: compact({
        id: merchant.id,
