...
function assertNoLegacyAuthFiles(dataDirectory: string): void {
  assert.equal(
    ...
    false,
    "PostgreSQL admin flow must not create merchants.json",
  );
  assert.equal(
    fs.existsSync(path.join(dataDirectory, "auth-security.json")),
    false,
    "PostgreSQL admin flow must not create auth-security.json",
  );
}

test("admin auth/security is end-to-end PostgreSQL authoritative", async (t) => {
  await seedOwner();
  const runtimeDirectory = await mkdtemp(
    path.join(os.tmpdir(), "fawri-admin-auth-postgres-proof-"),
  );
  ...
  const child = spawn(process.execPath, [serverEntry], {
    ...
    env: {
      ...
      META_VERIFY_TOKEN: "admin-auth-proof-meta-verify-token",
      META_APP_ID: "admin-auth-proof-meta-app",
      META_CONFIG_ID: "admin-auth-proof-meta-config",
  ...
  const ownerSessions = await json(await fetch(`${baseUrl}/api/auth/admin/sessions`, {
    headers: adminHeaders(ownerCookie, ownerDeviceId),
  }));
  assert.equal(ownerSessions.response.status, 200);
  assert.equal(ownerSessions.body?.sessions?.length, 1);
