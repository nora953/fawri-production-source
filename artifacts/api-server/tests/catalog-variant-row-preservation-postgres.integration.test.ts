...
process.env.FAWRI_AUTH_SECURITY_SECRET =
  "catalog-variant-row-preservation-secret-32-bytes-minimum";
...
async function createMerchant(suffix: string) {
  ...
  const pending = await accounts.upsertPendingMerchantAuthoritative({
    ...
    ownerName: `Row Preservation Owner ${suffix}`,
    storeName: `Row Preservation Store ${suffix}`,
    activityType: "retail",
    language: "ar",
...
async function createVariantProduct(params: {
  ...
  return catalog.createCatalogProductAuthoritative({
    ...
    idempotencyKey: `row-preservation-create-${params.suffix}`,
    input: {
      id: params.productId,
      name: "Variant row preservation proof",
      price_iqd: 10_000,
...
test("variant inventory mutations survive later catalog graph persistence", async () => {
  const suffix = crypto.randomUUID().replace(/-/g, "").slice(0, 12);
  const merchantId = await createMerchant(suffix);
  const productId = `prd-row-history-${suffix}`;
  const variantId = `var-row-history-${suffix}`;
  ...
  const first = await catalog.adjustCatalogInventoryAuthoritative({
    ...
    delta: -2,
    idempotencyKey: `row-history-adjust-1-${suffix}`,
  });
  const second = await catalog.adjustCatalogInventoryAuthoritative({
    ...
    delta: -1,
    idempotencyKey: `row-history-adjust-2-${suffix}`,
  });

  assert.equal(second.product.variants[0]?.id, variantId);
...
test("cashier sale evidence survives an ordinary product edit with the same variant identity", async () => {
  const suffix = crypto.randomUUID().replace(/-/g, "").slice(0, 12);
  const merchantId = await createMerchant(`cashier-${suffix}`);
  const productId = `prd-row-cashier-${suffix}`;
  const variantId = `var-row-cashier-${suffix}`;
  const created = await createVariantProduct({
    merchantId,
  ...
  const evidence = await raw(
    `SELECT request_hash, before_quantity, after_quantity
       FROM inventory_mutations
      WHERE merchant_id = $1 AND product_id = $2 AND variant_id = $3
  ...
  const requestHash = String(evidence.rows[0].request_hash);
  ...
  assert.equal(updated.variants[0]?.stock_quantity, 4);
  ...
  assert.equal(Number(after.rows[0].before_quantity), 5);
  assert.equal(Number(after.rows[0].after_quantity), 4);
});
