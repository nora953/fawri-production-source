...
test('history secondary controls share the reviewed light-gray surface in all languages', () => {
  assert.match(
    commerceFixes,
    /html\[data-cashier-view="history"\][\s\S]*div:first-child > div:last-child > button,[\s\S]*\.mb-3\.flex\.items-center\.justify-between\.gap-3 > button,[\s\S]*\.border-t > div:last-child > button:first-child \{[\s\S]*border: 1px solid #fecaca !important;[\s\S]*background: #f8fafc !important;[\s\S]*color: #b91c1c !important;[\s\S]*border-radius: 0\.75rem !important;[\s\S]*padding: 0\.625rem 1rem !important;[\s\S]*font-size: 0\.875rem !important;[\s\S]*font-weight: 700 !important;/,
  );
  assert.match(
    commerceFixes,
    /html\[data-cashier-view="history"\][\s\S]*button:first-child:hover \{[\s\S]*background: #f1f5f9 !important;/,
  );
});

test('history detail sync chip matches the outlined pill shape without forcing equal size', () => {
  assert.match(
    commerceFixes,
    /html\[data-cashier-view="history"\][\s\S]*div\.mb-1\.flex\.flex-wrap\.items-center\.gap-2 > span:last-child \{[\s\S]*border-style: solid !important;[\s\S]*border-width: 1px !important;[\s\S]*border-radius: 9999px !important;/,
  );
  assert.match(
    commerceFixes,
...
test('history sale-list sync state uses a compact outlined pill in all languages', () => {
  assert.match(
    commerceFixes,
    /html\[data-cashier-view="history"\] #cashier-root main section:first-child > div\.p-2 > button > div\.flex\.items-end\.justify-between\.gap-3 > span \{[\s\S]*border: 1px solid #a7f3d0 !important;[\s\S]*border-radius: 9999px !important;[\s\S]*padding: 0\.125rem 0\.5rem !important;/,
  );
  assert.match(
    commerceFixes,
...
test('Sorani void confirmation stays on one readable line without changing the reviewed wording', () => {
  assert.match(
    commerceFixes,
    /html\[lang="ku"\]\[data-cashier-view="history"\] \.fixed\.inset-0\.z-50 \.mt-5\.flex\.gap-2 > button:last-child \{[\s\S]*white-space: nowrap !important;[\s\S]*font-size: 0\.8125rem !important;[\s\S]*padding-inline: 0\.5rem !important;/,
  );
  assert.match(
    historyPage,
