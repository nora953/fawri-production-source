...
const TEXT: Record<Lang, Record<string, string>> = {
  ...
  ku: {
    ...
    back: 'گەڕانەوە بۆ کاشێر و کارمەندان', enabled: 'ڕێگەدان بە داشکاندنی دەستی',
  ...
  en: {
    ...
    back: 'Back to cashiers & staff', enabled: 'Allow manual discount',
...
export default function CashierDiscountPoliciesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const staffPayload = await staffResponse.json(); const policyPayload = await policyResponse.json();
      ...
      const nextPolicies: Record<string, PolicyRow> = {}; const nextDrafts: Record<string, Draft> = {};
      const fractionDigits = merchantSafeFractionDigits(regionalContext.currency_fraction_digits);
      for (const row of rows) { nextPolicies[row.staff_id] = row; nextDrafts[row.staff_id] = draftFromPolicy(row.discount_policy, fractionDigits); }
      setRegional(regionalContext); setStaff(staffList.filter(item => item.status !== 'revoked')); setPolicies(nextPolicies); setDrafts(nextDrafts);
    } catch { setMessage({ kind: 'error', text: copy.failed }); } finally { setLoading(false); }
  }, [copy.failed]);
  ...
  const updateDraft = (staffId: string, patch: Partial<Draft>) => setDrafts(current => ({ ...current, [staffId]: { ...current[staffId], ...patch } }));

  const save = async (member: Staff) => {
    const row = policies[member.id]; const draft = drafts[member.id];
    if (!row || !draft || !regional || savingId !== null) return;
    let percentageBps = 0; let amountMinor: number | null = null;
    if (draft.enabled) {
      const percentInput = draft.maxPercent.trim();
      if (percentInput === '') { setMessage({ kind: 'error', text: copy.percentRequired }); return; }
      const percent = Number(percentInput); const percentTimes100 = percent * 100;
      if (!Number.isFinite(percent) || percent < 0 || percent > 100 || !Number.isSafeInteger(Math.round(percentTimes100)) || Math.abs(percentTimes100 - Math.round(percentTimes100)) > 1e-7) { setMessage({ kind: 'error', text: copy.failed }); return; }
      percentageBps = Math.round(percentTimes100);
      amountMinor = merchantMoneyMajorInputToMinor(draft.maxAmount, fractionDigits);
      if (amountMinor === null || amountMinor <= 0) { setMessage({ kind: 'error', text: copy.amountRequired }); return; }
    }
    setSavingId(member.id); setMessage(null);
    try {
      const response = await fetch(`/api/cashier/management/staff/${encodeURIComponent(member.id)}/discount-policy`, {
        method: 'PUT', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ expected_version: row.staff_version, discount_policy: { enabled: draft.enabled, max_percentage_bps: percentageBps, max_amount_minor: amountMinor, can_approve_override: member.role === 'manager' && draft.enabled && draft.canApproveOverride } }),
      });
      const payload = await response.json(); if (!response.ok || payload.ok !== true) throw new Error(String(payload.code || 'SAVE_FAILED'));
      const nextRow: PolicyRow = { staff_id: member.id, staff_version: Number(payload.staff_version), discount_policy: payload.discount_policy as Policy };
    ...
    } catch { setMessage({ kind: 'error', text: copy.failed }); } finally { setSavingId(null); }
  };

  return <div className="mx-auto max-w-5xl space-y-5 p-1" dir={dir}>
    <div className="flex flex-wrap items-start justify-between gap-3"><div><h1 className="text-2xl font-black text-slate-950">{copy.title}</h1><p className="mt-1 text-sm text-slate-500">{copy.subtitle}</p></div><a href="/dashboard/cashiers" className="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-700 hover:bg-slate-50">{copy.back}</a></div>
    ...
    {loading ? <div className="rounded-2xl border bg-white p-8 text-center text-sm text-slate-500">{copy.loading}</div> : <div className="space-y-3">{visibleStaff.map(member => {
      ...
      return <section key={member.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        {draft.enabled ? <div className="mt-4 grid gap-3 md:grid-cols-2">
