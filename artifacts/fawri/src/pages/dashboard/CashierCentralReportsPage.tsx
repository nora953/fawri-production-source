...
function GroupCard({ name, secondary, report, lang, labels }: { name: string; secondary?: string; report: Report; lang: Lang; labels: Copy }) {
  return <div className="report-print-group-card rounded-xl border bg-background p-3"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-bold">{name}</p>{secondary ? <p className="mt-0.5 text-xs text-muted-foreground">{secondary}</p> : null}</div><span className="rounded-full bg-muted px-2.5 py-1 text-xs font-bold text-muted-foreground">{report.sale_count} {labels.sales}</span></div><div className="report-print-group-stats mt-3 grid gap-2 sm:grid-cols-2"><div className="rounded-lg border bg-card px-3 py-2"><p className="text-[11px] font-semibold text-muted-foreground">{labels.netSales}</p><div className="mt-1 text-sm font-bold"><MoneyStack values={moneyValues(report, 'net_revenue_minor')} lang={lang} /></div></div><div className="rounded-lg border bg-card px-3 py-2"><p className="text-[11px] font-semibold text-muted-foreground">{labels.profit}</p><div className="mt-1 text-sm font-bold"><MoneyStack values={profitValues(report)} lang={lang} /></div></div></div></div>;
}

function ActivityCard({ name, secondary, activity, labels }: { name: string; secondary?: string; activity: ActivityCounts; labels: Copy }) {
  return <div className="report-print-activity-card rounded-xl border bg-background p-3"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-bold">{name}</p>{secondary ? <p className="mt-0.5 text-xs text-muted-foreground">{secondary}</p> : null}</div><span className="rounded-full bg-muted px-2.5 py-1 text-xs font-bold text-muted-foreground">{labels.totalOps}: <b dir="ltr">{activity.operation_count}</b></span></div><div className="report-print-activity-stats mt-3 grid grid-cols-3 gap-2 text-center text-xs"><div className="rounded-lg border bg-card px-2 py-2"><p className="text-muted-foreground">{labels.saleOps}</p><p className="mt-1 text-base font-bold" dir="ltr">{activity.sale_count}</p></div><div className="rounded-lg border bg-card px-2 py-2"><p className="text-muted-foreground">{labels.returnOps}</p><p className="mt-1 text-base font-bold" dir="ltr">{activity.return_count}</p></div><div className="rounded-lg border bg-card px-2 py-2"><p className="text-muted-foreground">{labels.voidOps}</p><p className="mt-1 text-base font-bold" dir="ltr">{activity.void_count}</p></div></div></div>;
}
