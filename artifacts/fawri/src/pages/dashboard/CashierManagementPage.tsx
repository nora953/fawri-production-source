...
function knownPermissions(values: string[]): Set<StaffPermission> {
  const allowed = new Set<StaffPermission>([
    'sale.create', 'sale.view_own', 'sale.view_all', 'sale.return', 'sale.void', 'reports.sales', 'reports.profit',
  ]);
    ...
    <>
      ...
      <div className="mt-2 grid gap-2 sm:grid-cols-2">
        {permissionOptions.map(([permission, label]) => (
          <label
            key={permission}
            className={`flex items-start gap-2 rounded-lg border px-3 py-2 text-sm ${permission === 'reports.profit' ? 'border-amber-300 bg-amber-50/60 dark:bg-amber-950/10' : ''}`}
          >
            <input
              type="checkbox"
    ...
    <div className="flex flex-col gap-4 pb-4 xl:h-[calc(100dvh-4rem)] xl:min-h-0 xl:overflow-hidden xl:pb-0" dir={dir}>
                ...
                <div key={member.id} className="rounded-xl border bg-background p-3">
                  ...
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {member.permissions.map(permission => {
                      ...
                      return label ? (
                        <span key={permission} className={`rounded-full px-2 py-1 text-[11px] font-semibold ${permission === 'reports.profit' ? 'bg-amber-100 text-amber-800' : 'bg-muted text-muted-foreground'}`}>
                          {label}
                        </span>
                      ) : null;
                    })}
      ...
      {addStaffOpen ? (
        <Modal dir={dir} onClose={closeAddStaff}>
          ...
          <PermissionGrid value={permissions} toggle={permission => changePermission(setPermissions, permission)} />
      ...
      {editingMember ? (
        <Modal dir={dir} onClose={cancelEdit}>
          ...
          <PermissionGrid value={editPermissions} toggle={permission => changePermission(setEditPermissions, permission)} />
