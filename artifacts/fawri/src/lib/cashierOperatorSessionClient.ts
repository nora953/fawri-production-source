...
type ServerOperatorContext = Omit<
...
export async function pairCashierStation(
  ...
  // Pairing is allowed only with an empty outbox, so removing legacy raw cost
  // cannot destroy unsynchronized historical profit evidence.
...
export async function loginCashierOperator(
  ...
  const serverContext = payload.context as ServerOperatorContext;
  if (
    serverContext.station_id !== binding.station_id ||
    serverContext.device_id !== binding.device_id ||
    serverContext.merchant_id !== binding.merchant_id ||
    !serverContext.operator_session_id ||
    !serverContext.staff_id ||
    !serverContext.shift_id ||
    !Array.isArray(serverContext.permissions)
  ) {
    throw new CashierOperatorSessionClientError(
      'CASHIER_OPERATOR_LOGIN_RESPONSE_INVALID',
