...
export function createEmptyCatalogProductForm(): CatalogProductFormState {
  ...
  if (recovered) {
    ...
    return {
      ...
      service_location_modes: serviceLocationChoices(
        recoveredMode,
        recovered.service_location_modes,
      ),
    };
  }
  return {
...
export function catalogProductFormFromProduct(
  ...
  return {
    ...
    service_location_modes: serviceLocationChoices(
      service?.location_mode ?? 'merchant',
      service?.location_modes,
    ),
    name: product.name || '',
    sku: product.sku || '',
...
export function catalogProductInputFromForm(
  ...
  const compareAtPrice = serviceUsesAmount && form.original_price.trim()
    ...
    : null;
  const serviceLocationModes = serviceLocationChoices(
    form.service_location_mode,
    form.service_location_modes,
  );
