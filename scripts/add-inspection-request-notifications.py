...
# Backend: persist a dedicated inspection-request notification, emit realtime
# badge updates, and mark the notification read after the merchant decides.
...
''',
    "add merchant inspection notification types",
)
...
auth = replace_once(
    ...
    "use notification union in database",
)
...
type OtpRecord = {
''',
    "add notification record validator",
)
...
''',
    '''      merchant_notifications: Array.isArray(parsed.merchant_notifications)
        ...
        : [],
''',
    "normalize both notification types",
)
...
type MerchantRealtimeEventName =
''',
    "add inspection notification helper",
)
...
''',
    ...
    appendAdminLog(
''',
    "create notification with inspection request",
)
...
''',
    "emit inspection notification realtime update",
)
...
''',
    ...
    return res.json({ ok: true, ticket, inspection_request: inspectionRequest });
''',
    "mark inspection notification read after decision",
)
...
types_path = Path("artifacts/fawri/src/lib/types.ts")
...
''',
    "add frontend inspection notification type",
)
types_path.write_text(types_text, encoding="utf-8")


# Notification center: render inspection requests, localize them, mark them read,
# and open the exact support ticket.
notifications_path = Path("artifacts/fawri/src/pages/dashboard/NotificationsPage.tsx")
notifications_path.write_text(r'''import React, { useCallback, useEffect, useMemo, useState } from 'react';
...
# Deep-link the notification to the exact support ticket once, then remove the
# query parameter so later realtime refreshes do not override manual selection.
support_path = Path("artifacts/fawri/src/pages/dashboard/SupportPage.tsx")
...
''',
    "read notification ticket deep link",
)
support = replace_once(
    support,
...
''',
    "select support ticket from notification link",
)
support_path.write_text(support, encoding="utf-8")

print("Added persistent inspection request notifications, realtime badge updates, and ticket deep links.")
