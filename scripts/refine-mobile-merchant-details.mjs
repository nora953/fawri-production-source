...
function replaceOnce(segment, label, before, after) {
  const matches = segment.split(before).length - 1;
  if (matches !== 1) {
...
const channelsStartMarker = '          {activeTab === "channels" && (';
const channelsEndMarker = '          {activeTab === "notes" && (';
const channelsStart = detailsSection.indexOf(channelsStartMarker);
const channelsEnd = detailsSection.indexOf(channelsEndMarker, channelsStart);

if (channelsStart === -1 || channelsEnd === -1 || channelsEnd <= channelsStart) {
  throw new Error("Could not isolate channels tab");
}

let channelsSection = detailsSection.slice(channelsStart, channelsEnd);

channelsSection = replaceOnce(
  channelsSection,
  "channel card",
...
detailsSection =
  ...
  channelsSection +
  detailsSection.slice(channelsEnd);
